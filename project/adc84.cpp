...
#define NO_ADC  ((char)0xEE)

ATTimer timer;
uint32_t ledtime=_Sec(2);
uint32_t cycleTime=0;
int16_t analogVal=0xAAAA;
char getMux=ADCTEMPERATURE;
...
char setADC(char * cmd)
{
    ...
    if (x=='t') {
        ...
        x=NO_ADC;
    } else if (x>='0' && x<=(0x30 + ADCSINGLE_MAX)) {
        x=ADCn(x - 0x30);
    } else {
        return 0x45;
    }
